...
This app is designed to sync with a central server for configuration data. Clone the coconut-central couch to coconut-central-sijariemas.
Update the pointers to the relevant databases in coconut.config:

- coconut_central_url
- synchronization_target
- coconut_forms_url

When starting a new project, create a fresh synchronization_target, e.g. coconut-sijariemas-test. Clone (and give a unique name)
to coconut_central_url and coconut_forms_url

When testing locally, you may wish to point to a local instance of these couches. Alternately, you may wish to point to
a cloud instance of synchronization_target to test the quality/bandwidth of sync to a remote server.

Once you have the databases configured,  adjust the urls in LocalConfigView.coffee.

    <label for='coconut-cloud' >Coconut Cloud URL</label>
      <select name='coconut-cloud'>
        <option value='https://kiwicentral.org/coconut-central-sijariemas/'>https://kiwicentral.org/coconut-central-sijariemas/</option>
        <option value='http://192.168.16.101:5984/coconut-central-sijariemas'>http://192.168.16.101:5984/coconut-central-sijariemas</option>
        <option value='http://localhost:5984/coconut-central-sijariemas'>http://localhost:5984/coconut-central-sijariemas</option>
      </select>

The first time the app runs, select the url for coconut-central from the dropdown.
...
Set the default language for the admin user in coconut-forms-sijariemas/user.admin, langChoice.
