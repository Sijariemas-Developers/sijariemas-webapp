...
LocalConfigView = (function(_super) {
  ...
  LocalConfigView.prototype.events = {
    ...
    "click #reloadApp": "reloadApp"
  };

  LocalConfigView.prototype.reloadApp = function() {
  ...
  LocalConfigView.prototype.save = function() {
    ...
    if (coconutCloudCustom !== '') {
      coconutCloudConfigURL = coconutCloudCustom;
    } else {
    ...
    if (localConfig.mode && (coconutCloud != null)) {
        ...
        success: function(cloudConfig) {
          ...
          return Coconut.config.save(cloudConfig, {
            success: function() {
              ...
              $('#message').append("Replicating form definitions from " + Coconut.config.coconut_forms_url() + "<br/>");
              opts = {
                success: function() {
                  var repFromOpts;
                  $('#message').append("Replication of form definitions was successful.<br/>");
                  $('#message').append("Now attempting replication from server.<br/>");
                  repFromOpts = {
                    success: function() {
                      var repToOpts;
                      repToOpts = {
                      ...
                      return Coconut.syncView.sync.replicateToServer(repToOpts, completeExtra);
                    },
                    error: function(obj, msg) {
                      return $('#message').append("Replication Error:" + msg + "<br/>");
                    }
                  };
                  return Coconut.syncView.sync.replicateFromServer(repFromOpts, completeExtra);
                },
                error: function(obj, msg) {
              ...
              }, Coconut.syncView.sync.replicateForms(opts, completeExtra));
            },
      ...
      $('#message').append('Downloading configuration from <a href=\'' + coconutCloudConfigURL + '\'>' + coconutCloudConfigURL + '</a>');
